...
# Load the dataset
Markers <- read.csv("SGS_Markers_MOB.csv", header = TRUE, stringsAsFactors = FALSE)
...
write.table(hapmap_final, "Markers.hapmap", sep = "\t", quote = FALSE, row.names = FALSE)

write.csv(hapmap_final, "hapmap.csv", row.names = FALSE)


## Transforming hapmap format to numeric (Centered ) format

source("http://zzlab.net/GAPIT/gapit_functions.txt")  #install GAPIT

Data0 <- GAPIT(G=hapmap_final, output.numerical = T) #this spits out the numeric data (GD),
                                                        # Genetic map (GM)
                                                        # hapmap data (G)
                                                        #kinship matrix
Gen_data <- Data0$GD 

Gen_data <- Gen_data[,-1] #remove first column
